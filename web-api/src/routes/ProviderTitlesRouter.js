...
class ProviderTitlesRouter extends BaseRouter {
   ...
   */
  constructor(providerTitlesManager, tmdbManager, providerRepo, middleware) {
    super(middleware, 'ProviderTitlesRouter');
  ...
  initialize() {
     ...
     */
    this.router.get('/ignored', this.middleware.requireAuth, async (req, res) => {
      try {
        const {
          page = 1,
          per_page = 50,
     ...
     */
    this.router.get('/ignored/:id', this.middleware.requireAuth, async (req, res) => {
      try {
        const { id } = req.params;
     ...
     */
    this.router.post('/ignored/:id/validate-tmdb', this.middleware.requireAuth, async (req, res) => {
      try {
        const { id } = req.params;
     ...
     */
    this.router.put('/ignored/:id', this.middleware.requireAuth, async (req, res) => {
      try {
        const { id } = req.params;
