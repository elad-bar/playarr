...
/**
 * LiveTVProcessingManager for processing M3U and EPG files
...
export class LiveTVProcessingManager extends BaseProcessingManager {
  ...
  /**
   * Sync Live TV for multiple users
   * Processes M3U and EPG files, parses them, and stores via Domain Managers
  ...
  async syncUsers(users) {
    try {
      ...
      if (usernamesToSync.length > 0) {
        ...
        await Promise.all([
          this._channelManager.deleteChannelsByUsernames(uniqueUsernames),
          this._programManager.deleteProgramsByUsernames(uniqueUsernames)
        ]);
      }
      ...
      if (allChannels.length > 0) {
        ...
        await this._channelManager.insertChannels(allChannels);
      }

      if (allPrograms.length > 0) {
        this.logger.debug(`Inserting ${allPrograms.length} program(s)...`);
        await this._programManager.insertPrograms(allPrograms);
      }
      ...
      this.logger.error(`Error in syncUsers: ${error.message}`);
      throw error;
    }
  }
}
