# IPTV Provider Settings Wizard Redesign [Done]

## Overview

This feature redesigns the IPTV provider settings UI from a tab-based interface to a vertical wizard-based interface. The wizard replaces the tab navigation within the existing full-screen Dialog container, maintaining the same Dialog behavior (open/close flow, full-screen display). The wizard provides a step-by-step guided experience for adding and editing IPTV providers, with validation at each step, preview of upcoming steps, and the ability to save progress incrementally.
...
- Buttons: Continue only (no back button on first step, no save on first step)
...
- Buttons: Back, Continue, Save & Continue (middle step, not last)
...
**Step 4: Categories** (Xtream only, hidden for AGTV) - Last step in add mode
...
- Buttons: Back, Save, Save & Close (Finish)
...
- Buttons: Continue, Save & Continue (no back button on first step)

**Step 2: Cleanup Rules** (Xtream only)
- Same as Add Mode Step 3
- Buttons: Back, Continue, Save & Continue (middle step, not last)

**Step 3: Categories** (Xtream only) - Last step for Xtream providers
- Same as Add Mode Step 4
- Buttons: Back, Save, Save & Close (Finish)

**Step 4: Ignored Titles** - Last step for AGTV providers, or final step after Categories for Xtream
- Same as Add Mode Step 5
- Buttons: Back only (read-only step, no save needed)
...
- **First Step (Add Mode)**: Continue only
- **First Step (Edit Mode)**: Continue, Save & Continue
- **Middle Steps**: Back, Continue, Save & Continue
- **Last Step**: Back, Save, Save & Close (Finish)
...
- Available on all steps except the last step
- Validates current step
...
**Save Button:**
- Available on the last step only
- Validates current step
- Calls add/update API endpoint
- If successful: Updates local state, stays in wizard on current step
- If failed: Show error, keep current step expanded

**Save & Close / Finish Button:**
- Available on the last step only
- Validates current step
- Calls add/update API endpoint
- If successful: Updates local state, closes wizard, returns to provider cards view
- If failed: Show error, keep current step expanded
...
**Changes:**
- Keep the same full-screen Dialog container structure
- Replace tab-based navigation inside Dialog with ProviderWizard component
- Wizard replaces the tab interface but maintains the same Dialog behavior (full-screen, same open/close flow)
- Update provider list to open wizard instead of tabs-based dialog
